...
df_dem_all = [df_k3, df_k4, df_k5, df_k6, df_k7, df_k8]
...
def get_p_implicit(p, k, phi):
    I = get_I(p)
    p_star = p * (d / k)

    # term1 = phi_c * (1 - I / I_phi) * (1.0 + p_star / p_phi)
    # term2 = phi
    term1 = p_star / p_phi + I_phi * np.log(phi_c / phi)
    term2 = I
    sol = term1 - term2
    return sol


def solve_p_I(phi_stack, k):
    p_stack = []
    I_stack = []
    mu_stack = []
    q_stack = []
    ...
    for phi in phi_stack:
        sol = scipy.optimize.fsolve(get_p_implicit, 1e-12, args=(k, phi))
        p = sol
        # print(sol)
        I = get_I(p)
