...
def p2g(
    ...
    # interactions
    intr_masses = jnp.repeat(particles.masses, stencil_size).reshape(
        -1, 1, 1
    )
    intr_volumes = jnp.repeat(particles.volumes, stencil_size).reshape(
        -1, 1, 1
    )
    intr_velocities = jnp.repeat(
        particles.velocities, stencil_size, axis=0
    ).reshape(-1, dim, 1)
    intr_ext_forces = jnp.repeat(
        particles.forces, stencil_size, axis=0
    ).reshape(-1, dim, 1)
    intr_stresses = jnp.repeat(
        particles.stresses, stencil_size, axis=0
    ).reshape(-1, 3, 3)
...
def g2p(
    ...
    intr_vels = jnp.take(
        nodes_velocities, interactions.intr_hashes, axis=0,
        mode="fill",
        fill_value=0.0
    ).reshape(-1, dim, 1)
    ...
    intr_vels_nt = jnp.take(
        nodes_velocities_nt, interactions.intr_hashes, axis=0,
        mode="fill",
        fill_value=0.0
    ).reshape(-1, dim, 1)
    ...
    particles_velocities = (1.0 - alpha) * particle_vel_inc + alpha * (
        particles.velocities + particle_delta_vel_inc
    )
...
class USL(BaseSolver):
    ...
    def update(self: Self) -> Self:
        ...
        for mat in self.materials:
            particles, out_mat = mat.update_stress(
                particles=particles,
                dt=self.dt
            )
