...
def yield_function(p, q, H, cp, px0, M):
    return jnp.log(p / px0) + jnp.log(1 + (q**2) / (p**2 * M**2)) - H / cp
...
def get_e_m(p, q, N, M, lam, kap):
    m = q / p
    return N - lam * jnp.log(p) - (lam - kap) * jnp.log(1 + m**2 / M**2)
...
def get_H(q, p, Mf, M, deps_p_v, H_prev):
    m = q / p
    return ((Mf**4 - m**4) / (M**4 - m**4)) * deps_p_v + H_prev
...
class UHModel(Material):
    ...
    def vmap_update_stress(
        ...
        yf = yield_function(p_tr, q_tr, H_tr, cp, px0, self.M)
