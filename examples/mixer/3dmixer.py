...
def simulation_wrapper():
    ...
    from objects import HorizontalSemiCylinderSDF, MixerCylinderSDF
    ...
    pip_inner_radius = 4.5
    pip_outer_radius = 5.0
    # =========================================================
    # 2. DEFINE Water BODY p_idx 0, g_idx 0
    # =========================================================

    # sdf_block = hdx.BoxSDF(size=(5.0, 5.0,5.0))

    sdf_half_circle = HorizontalSemiCylinderSDF(
        length=6.0,         # Slightly shorter than pipe (10.0)
        radius=pip_inner_radius - (cell_size * 2), # Fits inside the inner radius (2.5)
        level_offset=-1.0 
    )

    position_stack = hdx.generate_particles_in_sdf(
        sdf_obj=sdf_half_circle,
        center_of_mass=jnp.array([5.0, 5.0, 5.0]),
        bounds_min=origin,
        bounds_max=end,
        cell_size=cell_size,
        ppc=ppc,
    ...
    water_c_id = sim_builder.add_constitutive_law(
        ...
        law=hdx.MuI_LC(K=2e6, mu_s=0.7, mu_d=1.41, I_0=1e-2, d_p=0.025, alpha=1e-4),
        density_stack=density_stack,
    )
    ...
    # cylinder = hdx.HollowCylinderSDF(
    #     outer_radius=pip_outer_radius, 
    #     inner_radius=pip_inner_radius,
    #     height=14.0,
    #     is_2d_ring=False,
    # )   

    cylinder = MixerCylinderSDF(
        outer_radius=pip_outer_radius, 
        inner_radius=pip_inner_radius,
        height=14.0,
        num_mixers=6,      # 6 Lifter bars
        bevel_height=1.2,  
        bevel_width=0.2    
    )
    ...
    # cos(45 deg) = 0.707...
    w = jnp.cos(theta / 2.0) 
    # sin(45 deg) = 0.707... * axis_z (1.0)
    z = jnp.sin(theta / 2.0)
    
    # Quaternion for 90 deg rotation around Z
    rot_sideways = jnp.array([w, 0.0, 0.0, z]) 

    spin_speed = 1.0 # Radians per second
    spin_vector = jnp.array([spin_speed, 0.0, 0.0])


    cylinder_state = cylinder.create_state(
        center_of_mass=jnp.array([5.0, 5.0, 5.0]),
        velocity=jnp.array([0.0, 0.0, 0.0]),
        rotation=rot_sideways,
        angular_velocity=spin_vector,
    )
    cylinder_idx = sim_builder.add_sdf_object(
        sdf_logic=cylinder,
        sdf_state=cylinder_state,
    )
    cylinder_collider_idx = sim_builder.add_sdf_collider(
        gap=1e-3,
        friction=0.7,
    )
    # =========================================================
    ...
    color = jnp.ones(len(position_stack)) * jnp.where(position_stack[:, 2] > 5.0, 1.0, 0.0)
    ...
    def log_simulation(sim_state: hdx.SimState):
        ...
        vis.log_sdf_boundary(
            sdf_logic=cylinder,
            sdf_state=sim_state.world.sdfs[cylinder_idx],
        )
