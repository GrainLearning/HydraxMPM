...
class SphereSDF(SDFObjectBase):
    ...
    def signed_distance_local(
        self, state: SDFObjectState, p_local: Float[Array, "dim"]
    ) -> Float[Array, ""]:
        # length(p) - r
        return jnp.linalg.norm(p_local) - self.radius

    def signed_distance(
        self, state: SDFObjectState, pos_world: Float[Array, "dim"]
    ) -> Float[Array, ""]:
...
class CapsuleSDF(SDFObjectBase):
    ...
    def signed_distance_local(
        ...
        vec_to_axis = p_local.at[1].set(p_local[1] - p_y_clamped)

        # Distance is dist(p, segment) - radius
        # dist = p_local - closest_on_segment
        # dist_to_axis = jnp.sqrt(jnp.sum(vec_to_axis**2) + 1e-14)
        
        return safe_norm(vec_to_axis)- self.radius
...
class StarSDF(SDFObjectBase):
    ...
    def __init__(
        self, points: int = 5, outer_radius: float = 1.0, inner_radius: float = 0.5
    ):
    ...
    def signed_distance_local(
        self, state: SDFObjectState, p: Float[Array, "dim"]
    ) -> Float[Array, ""]:
        ...
        angle = jnp.arctan2(
            p[0], p[1]
        )  # Align 0 with Y-axis (Standard star orientation)
        ...
        p_wedge = jnp.array([p[0] * c - p[1] * s, p[0] * s + p[1] * c])
        ...
        v2 = jnp.array(
            [self.inner_radius * jnp.sin(an), self.inner_radius * jnp.cos(an)]
        )
    ...
    def __init__(
        ...
        if isinstance(frictions, (float, int)):
            ...
            if self.frictions.shape[0] != num_walls:
                raise ValueError(
                    f"Expected {num_walls} frictions, got {self.frictions.shape[0]}"
                )
...
class MaterialPointCloudSDF(SDFObjectBase):
    """
    Stateless SDF representing a cloud of material points (spheres).
    ...
    def __init__(self, smooth_k=2.0):
        self.smooth_k = float(smooth_k)

    def get_normal(self, p_world, mp_state) -> Float[Array, "dim"]:
        """
        Calculates Normal for a single point using Autograd by default.
        """
        grad_fn = jax.grad(self.signed_distance, argnums=0)

        normal = grad_fn(p_world, mp_state)

        return normal / (jnp.linalg.norm(normal) + 1e-12)

    def signed_distance(self, p_world, mp_state) -> Float[Array, ""]:

        position_stack = mp_state.position_stack
        dim = position_stack.shape[1]
        volumes = mp_state.volume0_stack
        if dim == 2:
            # 2D: Area = pi * r^2  ->  r = sqrt(Area / pi)
            radii_particles = jnp.sqrt(volumes / jnp.pi + 1e-12)
        else:
            # 3D: Volume = 4/3 * pi * r^3  ->  r = cbrt(3*V / 4*pi)
            radii_particles = (3.0 * volumes / (4.0 * jnp.pi)) ** (1.0 / 3.0)

        diff = p_world - position_stack
        dists = jnp.linalg.norm(diff, axis=-1) - radii_particles

        if self.smooth_k > 1e-3:
            return -jax.nn.logsumexp(-self.smooth_k * dists) / self.smooth_k
        else:
            return jnp.min(dists)

    def get_signed_distance_stack(
        self, pos_world_stack: Float[Array, "n dim"], mp_state
    ) -> Float[Array, "n"]:
        """Vectorized interface for SDF calculation."""
        return jax.vmap(self.signed_distance, in_axes=(0, None))(
            pos_world_stack, mp_state
        )

    def get_normal_stack(
        self, pos_world_stack: Float[Array, "n dim"], mp_state
    ) -> Float[Array, "n dim"]:
        """Vectorized normals calculation."""
        return jax.vmap(self.get_normal, in_axes=(0, None))(pos_world_stack, mp_state)


# class ParticleCloudState(SDFObjectState):
#     """
#     Dynamic state for a particle cloud.
#     Holds the heavy arrays so JAX can update them without recompiling logic.
#     """
#     points: Float[Array, "num_points dim"]
#     radii: Float[Array, "num_points"]

# class ParticleCloudSDF(SDFObjectBase):
#     """
#     Logic for calculating distance to a cloud of points defined in the State.
#     """
#     smooth_k: float = eqx.field(static=True)
...
#         # Default COM to 0 so Local Space == World Space
#         if center_of_mass is None:
#             center_of_mass = jnp.zeros(points.shape[1])

#         base = super().create_state(center_of_mass=center_of_mass)
#         # points_local = points - center_of_mass
...
#     def signed_distance_local(self, state: ParticleCloudState, p_world, p_particles = None, radii_particles = None, volumes = None) -> Float[Array, ""]:

#         p_particles = state.points if p_particles is None else p_particles

#         if radii_particles is None and volumes is not None:
#             # Compute radii from volumes (2D circles)
#             dim = p_particles.shape[1]
#             if dim == 2:
#                 # 2D: Area = pi * r^2  ->  r = sqrt(Area / pi)
#                 radii_particles = jnp.sqrt(volumes / jnp.pi + 1e-12)
#             else:
#                 # 3D: Volume = 4/3 * pi * r^3  ->  r = cbrt(3*V / 4*pi)
#                 radii_particles = (3.0 * volumes / (4.0 * jnp.pi)) ** (1. / 3.0)

#         radii_particles = state.radii if radii_particles is None else radii_particles


#         diff = p_local - p_particles
#         dists = jnp.linalg.norm(diff, axis=-1) - radii_particles
#         # # 2. Combine
#         if self.smooth_k > 1e-3:
#             return -jax.nn.logsumexp(-self.smooth_k * dists) / self.smooth_k
...
#     ):
#         position_stack = mp_state.position_stack
#         volume_stack = mp_state.volume0_stack
#         dim = position_stack.shape[1]

#         # new_com = jnp.mean(position_stack, axis=0)

#         # position_stack = position_stack - new_com

#         if mp_state.dim == 2:
#             # 2D: Area = pi * r^2  ->  r = sqrt(Area / pi)
#             radius = jnp.sqrt(volume_stack / jnp.pi + 1e-12)
#         else:
#             # 3D: Volume = 4/3 * pi * r^3  ->  r = cbrt(3*V / 4*pi)
#             radius = (3.0 * volume_stack / (4.0 * jnp.pi)) ** (1. / 3.0)

#         radius = radius * radius_scale


#         return eqx.tree_at(
#             lambda s: (s.points, s.radii),
#             sdf_state,
#             (position_stack, radius)
#         )
